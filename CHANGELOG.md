...
**SurfManager v3.0** is a complete backend rewrite from Go + Wails v2 to Rust + Tauri v2 for maximum performance, minimal memory usage, and native system integration.

### ✨ Highlights

**Performance Improvements**
- Memory usage reduced from ~10MB to ~4MB at runtime
- Faster app startup and response times
- Zero garbage collection overhead (no GC in Rust)
- Native system WebView (no bundled Chromium)

**Technical Stack**
- **Backend**: Rust with Tauri v2
- **Frontend**: Svelte 3 + TailwindCSS (unchanged)
...
**Backend (Rust)**
- All Go code rewritten to idiomatic Rust
- Async/await with Tokio for file operations
- Strong type safety with serde serialization
- Error handling with `thiserror` crate
- Process management via `sysinfo` crate
- File dialogs via `rfd` crate
- ZIP operations via `zip` crate

**Build System**
- Cargo for Rust dependency management
- Tauri CLI for development and bundling
- MSI and NSIS installers for Windows
- Release profile with LTO optimization

### 📦 Migration Notes

- All user data remains compatible (same paths)
- App configs: `~/.surfmanager/AppConfigs/`
- Backups: `Documents/SurfManager/backup/`
- No migration required - just install and run

### 🔄 UI Changes

- Reset tab: Folder button moved next to Data Path
- Copy path button on the left, Folder button on the right
- Action buttons grid now 2 columns (Reset, Backup, Addons, New ID, Kill)
...
Session workflow upgrades, instant backups, and a cleaner VSCode preset layout.

### Added

- Sessions tab: Restore Account Only action
- Sessions tab: Replace Data with Current via right-click menu
- Reset tab: Instant backup modal for the selected app (full or addon)

### Changed

- Account-only restore now includes the Network folder
- VSCode preset merges User-related items into a single User entry
