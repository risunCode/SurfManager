<script>
  ...
  import { Plus, RefreshCw, Search, CheckSquare, FolderOpen, Trash2, RotateCcw, Package, Play, AlertTriangle, Loader2, Fingerprint } from 'lucide-svelte';
  import { GetActiveApps, GetApp, GetAllSessions, CreateBackup, RestoreBackup, RestoreAddonOnly, RestoreAccountOnly, CheckSessionHasAddons, DeleteSession, SetActiveSession, OpenSessionFolder, CountAutoBackups, LaunchApp, CalculateBackupSize, KillApp, ReplaceSessionData } from './api.js';
  ...
  let loadingRestoreAccount = {}; // { sessionKey: boolean }
  let loadingDelete = {};       // { sessionKey: boolean }
  let loadingReplace = {};      // { sessionKey: boolean }
  ...
  async function handleRestoreAccountOnly(session) {
    const sessionKey = `${session.app}/${session.name}`;
    if (loadingRestoreAccount[sessionKey]) return;
    ...
    if ($settings.confirmBeforeRestore) {
      ...
      message: `Restore account data from "${session.name}" for ${appConfig.display_name}?`,
    ...
    if (running) {
      ...
      if (!confirmed) return;
      loadingRestoreAccount = { ...loadingRestoreAccount, [sessionKey]: true };
      try {
        await killAppSilently(session.app);
      } catch (e) {
        const override = await confirm({
        ...
        if (!override) {
          loadingRestoreAccount = { ...loadingRestoreAccount, [sessionKey]: false };
          return;
        }
      }
    } else {
    ...
    log(`Restoring account files from session: ${session.name}...`);
    try {
      await RestoreAccountOnly(session.app, session.name, false);
      toast.success('Account files restored successfully');
      await loadData();
      loadingRestoreAccount = { ...loadingRestoreAccount, [sessionKey]: false };
      await promptLaunchIfAllowed(session.app);
    } catch (e) {
      ...
      if (msg.toLowerCase().includes('failed to close')) {
        ...
        if (override) {
          await RestoreAccountOnly(session.app, session.name, true);
          toast.success('Account files restored successfully');
          await loadData();
          loadingRestoreAccount = { ...loadingRestoreAccount, [sessionKey]: false };
          await promptLaunchIfAllowed(session.app);
      ...
      log(`Error: ${e}`);
      toast.error(`Account restore failed: ${e}`);
      loadingRestoreAccount = { ...loadingRestoreAccount, [sessionKey]: false };
    }
  }

  async function handleReplaceCurrent(session) {
    const sessionKey = `${session.app}/${session.name}`;
    if (loadingReplace[sessionKey]) return;

    const appConfig = await GetApp(session.app);

    const confirmed = await confirm({
      title: 'Replace Data with Current',
      message: `Replace contents of "${session.name}" with current data of ${appConfig.display_name}?`,
      confirmText: 'Replace',
      cancelText: 'Cancel',
      danger: true
    });
    ...
    if (running) {
      const killConfirmed = await confirm({
        title: `${appConfig.display_name} is Running`,
        message: 'Full replace requires the app to be closed.\n\nKill the app and continue, or cancel to perform addon-only replace?',
        confirmText: 'Kill App and Continue',
        cancelText: 'Addon-Only',
        danger: true
      });
      if (killConfirmed) {
        loadingReplace = { ...loadingReplace, [sessionKey]: true };
        try {
          await killAppSilently(session.app);
        } catch (e) {
          const override = await confirm({
          ...
          if (!override) {
            loadingReplace = { ...loadingReplace, [sessionKey]: false };
            return;
          }
        }
      } else {
    ...
    log(`Replacing session data for ${session.name}...`);
    try {
      await ReplaceSessionData(session.app, session.name, addonOnly);
      toast.success('Session data replaced successfully');
      await loadData();
      loadingReplace = { ...loadingReplace, [sessionKey]: false };
    } catch (e) {
      const msg = e?.toString?.() || '';
      if (!addonOnly && msg.toLowerCase().includes('must be closed')) {
        const override = await confirm({
          title: `${appConfig.display_name} is Running`,
          message: 'Full replace requires the app closed. Continue with addon-only replace?',
          confirmText: 'Addon-Only',
          cancelText: 'Cancel',
          danger: true
        });
        if (override) {
          try {
            await ReplaceSessionData(session.app, session.name, true);
            toast.success('Addon-only replace completed');
            await loadData();
            loadingReplace = { ...loadingReplace, [sessionKey]: false };
            return;
          } catch (ee) {
            log(`Error: ${ee}`);
            toast.error(`Replace failed: ${ee}`);
            loadingReplace = { ...loadingReplace, [sessionKey]: false };
            return;
          }
        }
  ...
  async function handleContextMenu(event, session) {
    ...
    const menuWidth = 220;
    const menuHeight = 320; // approximate height of menu content
    const padding = 8;
...
{#if contextMenu.show}
  ...
  >
    ...
    <button
      class="w-full px-4 py-2 text-left text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--warning)] transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
      on:click={() => { handleRestoreAccountOnly(contextMenu.session); closeContextMenu(); }}
      disabled={loadingRestoreAccount[`${contextMenu.session?.app}/${contextMenu.session?.name}`]}
    >
      {#if loadingRestoreAccount[`${contextMenu.session?.app}/${contextMenu.session?.name}`]}
        <Loader2 size={14} class="animate-spin" />
        Restoring...
      {:else}
        <Fingerprint size={14} />
    ...
    <button
      class="w-full px-4 py-2 text-left text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--primary)] transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
      on:click={() => { handleReplaceCurrent(contextMenu.session); closeContextMenu(); }}
      disabled={loadingReplace[`${contextMenu.session?.app}/${contextMenu.session?.name}`]}
    >
      {#if loadingReplace[`${contextMenu.session?.app}/${contextMenu.session?.name}`]}
        <Loader2 size={14} class="animate-spin" />
        Replacing...
      {:else}
        <RefreshCw size={14} />
        Replace Data with Current
      {/if}
    </button>
