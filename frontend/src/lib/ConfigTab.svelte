<script>
  ...
  import { CheckAppInstalled, GetApp, GetApps, OpenConfigFolder, SaveApp, DeleteApp, SelectExeFromLocalPrograms, SelectFolderFromHome, SelectFolderFromRoaming, SelectFolderFromLocalPrograms, ToggleApp, GetPlatformInfo } from './api.js';
  ...
  const vscodePresetItems = [
    ...
    { type: 'folder', path: 'User', description: 'User data (settings, keybindings, global storage)', optional: false, enabled: true },
  ];
  const legacyUserPaths = new Set([
    'User/settings.json',
  ...
  async function openEditDialog(app) {
    ...
    const isVscodePreset = existingItems.some(item => 
      vscodePresetItems.some(preset => preset.path === item.path) || legacyUserPaths.has(item.path)
    );
    ...
    if (isVscodePreset && existingItems.length > 0) {
      ...
      const hasLegacyUser = existingItems.some(item => legacyUserPaths.has(item.path));
      newApp.backup_items = vscodePresetItems.map(preset => {
        const existing = existingItems.find(e => e.path === preset.path);
        return {
          ...preset,
          enabled: preset.path === 'User' ? (!!existing || hasLegacyUser) : !!existing
        };
      });
      // Add any custom items that aren't in preset
      existingItems.forEach(item => {
        if (!vscodePresetItems.find(p => p.path === item.path) && !legacyUserPaths.has(item.path)) {
      ...
      const userIndex = allItems.findIndex(item => item.path === 'User');
      ...
      existingItems.forEach(item => {
        ...
        } else if (legacyUserPaths.has(item.path) && userIndex >= 0) {
          allItems[userIndex].enabled = true;
        } else {
